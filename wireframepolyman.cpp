...
void loadpolyman(float px[][4], float py[][4], float pz[][4], float pex[], float pey[], float lx[], float ly[], float lz[])
{
    ...
    px[1][0] = -9.0f / 8.0f; py[1][0] = 0.0f;         pz[1][0] = -1.0f;
    px[1][1] = -5.0f / 8.0f; py[1][1] = 3.0f / 4.0f;  pz[1][1] = -1.0f;
    px[1][2] = 5.0f / 8.0f;  py[1][2] = 3.0f / 4.0f;  pz[1][2] = -1.0f;
    px[1][3] = 9.0f / 8.0f;  py[1][3] = 0.0f;         pz[1][3] = -1.0f;
    ...
    lx[4] = 0.25f;  ly[4] = -0.5f; lz[4] = -1;
    lx[5] = 0.25f;  ly[5] = -1.0f; lz[5] = -1;
    lx[6] = 0.25f;  ly[6] = -1.0f; lz[6] = -1;
    lx[7] = 0.0f;   ly[7] = -1.0f; lz[7] = -1;
...
void drawpolyman(float px[][4], float py[][4], float pz[][4], float pex[], float pey[], float lx[], float ly[], float lz[]) {
    ...
    for (int i = 0; i < 4; i++) {
        glVertex3f(px[0][i], py[0][i], pz[0][i]);
    }
    glEnd();
    ...
    glBegin(GL_LINE_LOOP);
    for (int i = 0; i < 4; i++) {
        glVertex3f(px[1][i], py[1][i], pz[1][i]);
    }
    glEnd();
    ...
    if (mouthOpenMan) {
        ...
        // Connect front and back with lines
        glBegin(GL_LINES);
        glVertex3f(9.0f / 8.0f, 0.0f, 1.0f);  glVertex3f(9.0f / 8.0f, 0.0f, -1.0f);
        glVertex3f(5.0f / 8.0f, -3.0f / 4.0f, 1.0f);  glVertex3f(5.0f / 8.0f, -3.0f / 4.0f, -1.0f);
        glVertex3f(-5.0f / 8.0f, -3.0f / 4.0f, 1.0f);  glVertex3f(-5.0f / 8.0f, -3.0f / 4.0f, -1.0f);
        glVertex3f(-3.0f / 8.0f, 0.0f / 4.0f, 1.0f);  glVertex3f(-3.0f / 8.0f, 0.0f / 4.0f, -1.0f);
        glVertex3f(-9.0f / 8.0f, 0.0f, 1.0f);  glVertex3f(-9.0f / 8.0f, 0.0f, -1.0f);
        glEnd();
...
void loadpolywoman(float pwx[][4], float pwy[][4], float pwz[][4], float pwex[], float pwey[], float lwx[], float lwy[], float lwz[])
{
    ...
    pwx[1][0] = -9.0f / 8.0f; pwy[1][0] = 0.0f;         pwz[1][0] = -1.0f;
    pwx[1][1] = -5.0f / 8.0f; pwy[1][1] = 3.0f / 4.0f;  pwz[1][1] = -1.0f;
    pwx[1][2] = 5.0f / 8.0f;  pwy[1][2] = 3.0f / 4.0f;  pwz[1][2] = -1.0f;
    pwx[1][3] = 9.0f / 8.0f;  pwy[1][3] = 0.0f;         pwz[1][3] = -1.0f;
    ...
    lwx[4] = 0.25f;  lwy[4] = -0.5f; lwz[4] = -1;
    lwx[5] = 0.25f;  lwy[5] = -1.0f; lwz[5] = -1;
    lwx[6] = 0.25f;  lwy[6] = -1.0f; lwz[6] = -1;
    lwx[7] = 0.0f;   lwy[7] = -1.0f; lwz[7] = -1;
...
void settranspolyman()
{
    ...
    glRotatef(thetaPolyman, 0.0f, 1.0f, 1.0f);
...
void TimerFunction(int value)
{
    ...
    case 3:
        ...
        break;
    case 4:
        thetaPolyman -= 1.0f; // Rotate during the backflip
